...
const NATION_NAMES: Record<string, string> = {
  ...
  natlan: '纳塔', nodkrai: '挪德卡莱', snezhnaya: '至冬', traveler: '旅行者篇', unknown: '未归属地区',
}
  ...
  return <main className="catalog-page">
    ...
    {catalogView === 'cards' && <section className="catalog-controls">
      ...
      <div className="filter-row">
        ...
        <MultiFilter id="versions" open={openPopover==='versions'} onOpen={()=>setOpenPopover(openPopover==='versions'?null:'versions')} icon={<Clock3 size={15}/>} label="发布版本" values={versions} onChange={(next)=>{setVersions(next);setLimit(60)}} options={[...data.versions.map((v)=>[v,`v${v}`]),['unknown','版本数据缺失']]}/>
...
function SceneBlock({ scene, sceneIndex, mode, languages, traveler, selected, toggle, selecting, query, matches, focusedKey, showGuide }: { scene: Scene; sceneIndex: number; mode: ViewMode; languages: LanguageCode[]; traveler: Traveler; selected: Set<string>; toggle: (k: string) => void; selecting: boolean; query: string; matches: Set<string>; focusedKey?: string; showGuide:boolean }) {
  ...
  while (index < scene.lines.length) {
    ...
    if (line.branchGroupId) {
      const start=index;const group:DialogueLine[]=[];while(index<scene.lines.length&&scene.lines[index].branchGroupId===line.branchGroupId)group.push(scene.lines[index++]);const total=line.branchTotal||new Set(group.map((item)=>item.branchIndex)).size;const flow=line.branchFlow|| (group.some((item)=>item.branchRole==='response')?'divergent':'convergent');const flowCopy=flow==='loop'?['可重复问答','每个问题有独立回答；选择结束项后离开问答']:flow==='divergent'?['差异分支','每个选项及其专属回应分别列出，随后回到共同剧情']:flow==='independent'?['独立分支','各选项沿独立路径结束，不存在共同汇合节点']:flow==='unresolved'?['路径数据不完整','源数据在该分支末端未提供可继续追踪的下一节点']:['旅行者选项','不同说法会立即回到相同后续']
      blocks.push(<section className={`choice-group choice-${flow}`} key={`branch:${line.branchGroupId}`}><header><GitFork size={16}/><div><strong>{flowCopy[0]}</strong><small>{total} 个选项 · {flowCopy[1]}</small></div></header><div className="branch-paths">{Array.from({length:total},(_,branchIndex)=>{const path=group.filter((item)=>item.branchIndex===branchIndex);return <section className={`branch-path branch-tone-${branchIndex%4}`} key={branchIndex}><div className="branch-path-label"><b>{branchIndex+1}/{total}</b><span>{flow==='loop'?(branchIndex===total-1?'结束问答':'问题与回答'):flow==='divergent'?'分支路径':'可选说法'}</span></div>{path.map((item,pathIndex)=><DialogueRow key={item.key} line={item} index={start+pathIndex} optionIndex={pathIndex===0?branchIndex:undefined} optionTotal={pathIndex===0?total:undefined} mode={mode} languages={languages} traveler={traveler} checked={selected.has(item.key)} toggle={()=>toggle(item.key)} selecting={selecting} query={query} match={!query||matches.has(item.key)} focused={item.key===focusedKey}/>)}</section>})}</div></section>)
      if(line.branchMergeNodeId&&index<scene.lines.length)blocks.push(<div className="common-story-marker" key={`common:${line.branchGroupId}`}><span>以上 {total} 条路径在此汇合，以下为共同后续</span></div>);continue
    }
    if (line.kind !== 'choice') {
  ...
  return <section className="scene-block" data-scene-key={scene.key}><header><span>SCENE {String(sceneIndex + 1).padStart(2,'0')}</span><div><h3>{localized(scene.title, languages[0])}</h3>{languages.slice(1).map((lang) => <p title={localized(scene.title,lang)} key={lang}>{localized(scene.title, lang)}</p>)}</div><em>{scene.lines.length} 句</em></header>{tipVisible && <aside className="scene-lead"><Info size={14} /><div><strong>{scene.description.zh ? '本节提示' : '场景阅读提示'}</strong><p>{scene.description.zh ? localized(scene.description,languages[0]) : '选项路径依据游戏对话节点的下一跳关系整理，并明确标出立即汇合、专属回应、循环问答或独立结束。'}</p></div><button aria-label="关闭本场景提示" onClick={() => { sessionStorage.setItem(guideKey,'done'); setTipVisible(false) }}><X size={14} /></button></aside>}{blocks}</section>
}
  ...
  return <div ref={ref} className={`print-document density-${settings.density} layout-${printLayout} color-${settings.color} speaker-${settings.speakerLayout || 'column'} ${settings.lineNumbers ? '' : 'no-line-numbers'}`} style={{ '--doc-font': `${settings.fontSize}pt`, '--speaker-font':`${settings.speakerSize ?? 7}pt`, '--speaker-width':`${settings.speakerWidth ?? 14}mm`, '--number-font':`${settings.numberSize ?? 6}pt`, '--scene-title-font':`${settings.sceneTitleSize ?? 9}pt`, '--cover-title-font':`${settings.coverTitleSize ?? 15}pt`, '--line-gap':`${settings.lineGap ?? 1}mm`, '--scene-gap':`${settings.sceneGap ?? 1.5}mm`, '--print-language-count': shownLanguages.length } as React.CSSProperties}>
    ...
    {bundles.map((bundle, bundleIndex) => <section className="print-source" key={bundle.key}>
      ...
      {bundle.scenes.map((scene, si) => {
        ...
        if(line.branchGroupId){optionIndex=line.branchIndex??0;optionTotal=line.branchTotal??0}
        const ratio = settings.columnRatio ?? 50
        ...
        return <div className={`print-line kind-${line.kind} ${optionTotal > 1 ? `choice-option choice-tone-${optionIndex % 4}` : ''} ${line.branchRole?`branch-${line.branchRole} branch-${line.branchFlow}`:''} ${line.kind === 'choice' && optionTotal > 1 && optionIndex === 0 ? 'choice-start' : ''} ${repeatedSpeaker ? 'same-speaker' : ''}`} style={{ gridTemplateColumns:columns }} key={line.key}>{settings.lineNumbers && <span className="print-number"><b>{line.branchRole==='option'&&optionTotal>1?`${optionIndex + 1}/${optionTotal}`:line.branchRole==='response'?'回应':String(li + 1).padStart(3,'0')}</b><small>{overall}/{lineCount}</small></span>}{shownLanguages.map((lang) => <div className={`print-cell lang-${lang.toLowerCase()}`} key={lang}>{settings.speakers && !repeatedSpeaker && localized(line.speaker, lang) && <strong>{localized(line.speaker, lang)}</strong>}<p>{formatGameText(localized(line.text, lang), traveler)}</p></div>)}</div>
        })
