...
type BranchMark=Pick<DialogueLine,'branchGroupId'|'branchIndex'|'branchTotal'|'branchRole'|'branchFlow'|'branchMergeNodeId'>
...
export function enrichBranches(lines:DialogueLine[]):DialogueLine[] {
  ...
  for(const selector of lines.filter((line)=>line.nodeId.endsWith('-player')&&line.sourceType==='MultiDialog')) {
    ...
    const tails=paths.map((path)=>nodes.get(path.at(-1)||'')?.nextNodeId||'')
    const returns=tails.map((next)=>next===selector.nodeId)
    const uniquePaths=paths.map((path)=>path.slice(0,merge?path.indexOf(merge):undefined))
    const flow:DialogueLine['branchFlow']=returns.some(Boolean)
      ?'loop'
      :merge
        ?uniquePaths.some((path)=>path.length>1)?'divergent':'convergent'
        :tails.every((next)=>next==='finish')?'independent':'unresolved'
    uniquePaths.forEach((path,index)=>path.forEach((id,step)=>marks.set(id,{branchGroupId:selector.nodeId,branchIndex:index,branchTotal:targets.length,branchRole:step===0?'option':'response',branchFlow:flow,branchMergeNodeId:merge})))
  }
