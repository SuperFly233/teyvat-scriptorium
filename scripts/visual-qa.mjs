...
await capture('reader-search-locate', '/?chapter=1700', { width: 1440, height: 900 }, { action: async (page) => { await page.locator('.reader-search input').fill('派蒙'); await page.waitForTimeout(300); console.log('search-locate', await page.evaluate(() => ({ marks: document.querySelectorAll('mark').length, muted: document.querySelectorAll('.search-muted').length, focused: document.querySelectorAll('.search-focused').length }))) } })
await capture('reader-role-filter', '/?chapter=1700', { width: 390, height: 844 }, { action: async (page) => { await page.locator('.role-filter > button').click(); await page.locator('.role-filter-list input').nth(1).uncheck() } })
await capture('print-studio', '/?chapter=1700', { width: 1440, height: 900 }, { action: async (page) => { await page.locator('.add-to-basket').click(); await page.locator('.desktop-print-fab').click() } })
...
await historyPage.locator('.catalog-search input').fill('1700')
await historyPage.locator('.catalog-card').first().click()
await historyPage.waitForSelector('.reader-page')
const readerUrl = historyPage.url()
await historyPage.goBack(); await historyPage.waitForSelector('.catalog-page')
const backUrl = historyPage.url()
await historyPage.goForward(); await historyPage.waitForSelector('.reader-page')
console.log('history-navigation', JSON.stringify({ readerUrl, backUrl, forwardUrl: historyPage.url() }))
...
await pdfPage.locator('.add-to-basket').click()
...
await exportPage.locator('.add-to-basket').click()
...
const ultraExportPage = await browser.newPage({ viewport: { width: 1440, height: 900 }, acceptDownloads: true })
await ultraExportPage.goto(`${baseUrl}/?chapter=1700`, { waitUntil: 'networkidle' })
await ultraExportPage.locator('.add-to-basket').click()
await ultraExportPage.locator('.desktop-print-fab').click()
await ultraExportPage.locator('.segment button', { hasText: '超紧凑' }).click()
const ultraDownloadPromise = ultraExportPage.waitForEvent('download', { timeout: 120000 })
await ultraExportPage.locator('.primary-action').click()
const ultraDownload = await ultraDownloadPromise
await ultraDownload.saveAs(resolve('tmp/pdfs/direct-export-ultra.pdf'))
console.log('direct-export-ultra', JSON.stringify({ filename: ultraDownload.suggestedFilename() }))
await browser.close()
